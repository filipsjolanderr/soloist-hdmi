...
**HDMI cuts out for a few seconds, repeatedly.** Not a buffer problem and not
the receiver. Check whether the client is reconnecting:

```bash
journalctl -u snapserver --since -30min | grep -c 'onDisconnect: hifi-hdmi'
```

Anything above zero over a quiet half hour is the fault. Every reconnect stops
and restarts the player, and that gap is what you hear. The client log names the
trigger:
...
The cause is **wifi power save**, which `brcmfmac` enables by default:

```bash
/usr/sbin/iw dev wlan0 get power_save     # "on" is the bug
```

It parks the radio between beacons, so a 2 ms LAN round trip becomes an
occasional 70 ms one; snapcast's time sync gives up on the spike and tears the
session down. `install.sh` disables it via a NetworkManager `conf.d` drop-in
rather than the connection profile, because the profile here is generated by
netplan and anything written into it is lost on the next regeneration.

Worth knowing what it is *not*: the Pi Zero 2 W is 2.4 GHz only, so moving to
5 GHz is not an option, and its link here negotiates a 5.5 Mbit/s rx rate, which
is poor but still eight times what a 44.1/16/2 FLAC stream needs. If the
reconnects persist with power save off, the link itself is the problem — signal,
placement, or 2.4 GHz congestion — and the mitigation is snapserver's `buffer`
(1000 ms by default), which trades start-up latency for tolerance of stalls.
